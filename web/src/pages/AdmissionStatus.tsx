...
export default function AdmissionStatus() {
  ...
  const submitDefense = async () => {
    if (defenseText.trim().length < 10) {
      setError('La defensa debe tener al menos 10 caracteres.')
      return
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al enviar defensa')
    } finally {
  ...
  if (loading) {
    return (
      <div className="max-w-2xl mx-auto py-12 text-center">
        ...
        <p className="text-xs text-gray-500 mt-3">Cargando estado...</p>
      </div>
    )
  }
  ...
  if (!status) {
    return (
      <div className="max-w-2xl mx-auto py-12 text-center space-y-3">
        ...
        <p className="text-sm text-gray-600">{error || 'No se encontró solicitud de admisión.'}</p>
      </div>
    )
  }
  ...
  const statusConfig: Record<string, { icon: any; color: string; bg: string; title: string; desc: string }> = {
    pending_review: {
      ...
      desc: 'Tu solicitud está siendo revisada por la comisión de ingresos. Te notificaremos cuando haya novedades.',
    },
    elevated_to_assembly: {
      icon: Gavel, color: 'text-blue-700', bg: 'bg-blue-50 border-blue-200',
      title: 'Elevada a la Asamblea',
      desc: 'Tu solicitud fue aprobada para discutirse en asamblea. Se votará en la próxima asamblea.',
    },
    approved: {
      icon: CheckCircle2, color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200',
      title: '¡Aprobado! Bienvenido a la comunidad',
      desc: 'Has sido aprobado como miembro. Ya tienes acceso completo al sistema.',
    },
    rejected: {
      icon: XCircle, color: 'text-red-700', bg: 'bg-red-50 border-red-200',
      title: 'Solicitud Rechazada',
      desc: 'Tu solicitud fue rechazada. Puedes enviar una defensa explicando tu caso.',
    },
    defense_pending: {
      icon: Shield, color: 'text-purple-700', bg: 'bg-purple-50 border-purple-200',
      title: 'Defensa en Revisión',
      desc: 'Tu defensa fue enviada y está siendo revisada por la comisión.',
    },
  }
  ...
  return (
    <div className="max-w-2xl mx-auto py-6 space-y-5">
      <div className="text-center space-y-1">
        <h1 className="text-2xl font-extrabold text-gray-900">Estado de tu Solicitud</h1>
        <p className="text-xs text-gray-500">Sigue el progreso de tu postulación a la comunidad</p>
      </div>
      ...
      <div className={`rounded-2xl p-6 border-2 ${cfg.bg} space-y-3`}>
        ...
        {status.next_assembly && status.status === 'elevated_to_assembly' && (
          <div className="flex items-center gap-2 text-xs text-blue-700 bg-white rounded-lg p-2.5 border border-blue-100">
            <Calendar size={15} />
            <span>Próxima asamblea: <b>{new Date(status.next_assembly).toLocaleString('es')}</b></span>
          </div>
        )}

        {status.rejection_reason && (
          <div className="bg-white rounded-lg p-3 border border-red-100 text-xs space-y-1">
            <b className="text-red-800 block">Motivo del rechazo:</b>
        ...
        {daysLeft !== null && status.status === 'rejected' && (
          <div className="flex items-center gap-2 text-xs text-red-700 bg-white rounded-lg p-2.5 border border-red-100">
            <Clock size={15} />
            <span>Te quedan <b>{daysLeft} días</b> para enviar una defensa antes de que tu cuenta sea eliminada.</span>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 border border-gray-200 space-y-3">
        ...
        <div className="space-y-2 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-700">Solicitud enviada: {status.submitted_at ? new Date(status.submitted_at).toLocaleString('es') : '—'}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${status.elevated_at ? 'bg-blue-500' : 'bg-gray-300'}`} />
            <span className="text-gray-700">Elevada a asamblea: {status.elevated_at ? new Date(status.elevated_at).toLocaleString('es') : '—'}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${status.defense_submitted_at ? 'bg-purple-500' : 'bg-gray-300'}`} />
            <span className="text-gray-700">Defensa enviada: {status.defense_submitted_at ? new Date(status.defense_submitted_at).toLocaleString('es') : '—'}</span>
          </div>
        </div>
      </div>
      ...
      {status.status === 'rejected' && daysLeft !== null && daysLeft > 0 && !status.defense_text && (
        <div className="bg-white rounded-2xl p-5 border border-purple-200 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-purple-900">Enviar Defensa</h3>
          </div>
          <p className="text-xs text-gray-600">
            Si crees que el rechazo fue un malentendido, puedes explicar tu caso. La comisión revisará tu defensa.
          </p>
          <textarea
            rows={4}
            className="input text-xs sm:text-sm"
            placeholder="Explica por qué crees que tu solicitud debería ser reconsiderada..."
            value={defenseText}
          ...
          >
            {submittingDefense ? 'Enviando...' : 'Enviar Defensa'}
            <ArrowRight size={14} />
          </button>
        </div>
      )}
      ...
      {status.defense_text && (
        <div className="bg-white rounded-2xl p-5 border border-purple-200 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-purple-900">Tu Defensa</h3>
            {status.defense_status && (
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                ...
                {status.defense_status === 'accepted' ? 'Aceptada' :
                 status.defense_status === 'rejected' ? 'Rechazada' : 'En revisión'}
              </span>
            )}
          </div>
