    ...
    <div className="space-y-4">
      ...
      {showHelp && (
        <div className="card bg-blue-50 border-blue-200 text-sm text-gray-700 space-y-2">
          ...
          <button onClick={() => setShowHelp(false)} className="text-blue-600 underline">{t('admission.close', 'Cerrar')}</button>
        </div>
      )}
      ...
      {showForm && (
        <div className="card space-y-4">
              ...
              <div className="space-y-2 mb-3">
                {documents.map((doc, i) => (
                  <div key={i} className="flex items-center justify-between bg-gray-50 p-2 rounded-lg">
                    ...
                    <button type="button" onClick={() => removeDocument(i)} className="text-red-500 hover:text-red-700 text-xs">{t('admission.delete', 'Eliminar')}</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowForm(false)} className="btn-secondary">{t('admission.cancel', 'Cancelar')}</button>
          </div>
        </div>
      )}
